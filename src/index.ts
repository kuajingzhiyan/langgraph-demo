...
function buildDeleteInterrupt(toolCall: {
  name: string;
  args: Record<string, unknown>;
}): HITLRequest {
  return {
    action_requests: [
      {
        ...
        args: toolCall.args,
        description:
          "This action will delete RoxyBrowser window(s). Please confirm before continuing.",
      },
    ],
    review_configs: [
      {
...
function createRejectionToolMessage(
  toolCallId: string,
  toolName: string,
): ToolMessage {
...
async function chat(
  ...
  if (isRejectedToolMessage(lastMessage)) {
    return {
      messages: [
        new AIMessage({
          content:
            "删除操作已被用户拒绝，我将停止执行此操作。如需继续，请重新发起请求。",
        }),
      ],
    };
  }
  ...
  const message = await generateWithAnthropic(messages, config, mcpTools);
  ...
  for (const toolCall of toolCalls) {
    ...
    if (isDeleteWindowTool(toolCall.name)) {
      const resumeValue = interrupt(
        buildDeleteInterrupt({ name: toolCall.name, args }),
      );
      const decision = getDecision(resumeValue);

      if (!decision || decision.type === "reject") {
        toolMessages.push(
          createRejectionToolMessage(toolCall.id!, toolCall.name),
        );
        break;
      }
...
function shouldContinue(
  ...
  return aiMessage.tool_calls && aiMessage.tool_calls.length > 0
    ? "tools"
    : END;
}
